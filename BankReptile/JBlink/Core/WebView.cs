...
namespace JBlink
{
    public class WebView : IDisposable
    {
        ...
        {
            uint flags = 0;
            if (((keyCode >> 16) & (int)WinConst.KF_REPEAT) != 0)
                flags |= (uint)wkeKeyFlags.WKE_REPEAT;
            if (((keyCode >> 16) & (int)WinConst.KF_EXTENDED) != 0)
                flags |= (uint)wkeKeyFlags.WKE_EXTENDED;

            //var result = MBApi.wkeFireKeyDownEvent(m_WebView, keyCode, flags, false);
            //result = MBApi.wkeFireKeyUpEvent(m_WebView, keyCode, flags, false);
            MBApi.wkeFireKeyPressEvent(m_WebView, keyCode, flags, false);
        }
